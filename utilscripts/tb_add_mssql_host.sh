...
"params": {
          ...
          "macros": [ { "macro": "{$DB_INST}", "value": "'$oid'" }],
          "templates": [ {"templateid":"11545"} ]
          }
}' http://zabbix/api_jsonrpc.php `
echo $hostcreate
#create ZIS Event Unit
zexec zismain -tcp  "CMD 0301 rexusr REXX ZEXEC_Create_EU \"$host DB-MSSQL\" \"DiscoveredApplications\" " > /dev/null 2>&1
fi
